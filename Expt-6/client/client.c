...
//for including structures which will store information needed #include <netinet/in.h>
#include <unistd.h>
...
int main()
{
    ...
    //if the gethostname returns a name then the program will get the ip address if(gethostname(hostname,sizeof(hostname))==0){
    hostIP = gethostbyname(hostname); //the netdb.h fucntion gethostbyname }else{
...
{                                                                      //recieve the data from the server
    recv(socketDescriptor, serverResponse, sizeof(serverResponse), 0); //recieved data from the server successfully then printing the data  obtained from the server
